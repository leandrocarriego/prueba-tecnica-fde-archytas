...
ejecuta cada tarea**. La numeración es global y sigue la ola: se ejecuta por número, de 2 a 44.

**No hay tarea 1, y el hueco es a propósito.** Era separar los cimientos del sistema de diseño en
una rama y commitearlos; el **PR #29** los llevó a `main` —tokens, primitivas, `design-system.test.ts`,
`vitest.config.ts`, `docs/design/README.md`, el bloque `UI-*` de `CONVENTIONS.md` y las
devDependencies con su lock— y la dejó sin objeto. La numeración no se corre: las referencias
cruzadas de este documento y del plan apuntan a estos números.

**Cómo se lee la columna `#`:** ✅ hecha y verificada. Las cuarenta y tres están hechas.

Los cuatro recorridos con Playwright (13, 21, 30 y 43) se corrieron el 2026-09-01 contra el build
de producción, con un acceso de dueño y uno de Ventas y el sistema operativo en modo oscuro: 100
verificaciones en verde, y **dos defectos que ninguna aserción veía y que se corrigieron**. La
evidencia, el guion y el juego de datos están en
[`evidence/`](./evidence/README.md).

| Ola | Qué | Tareas |
|---|---|---|
| **0 · Cimientos** | Tokens, `tone.ts`, `amount.tsx`, `state.tsx` y los cinco chequeos nuevos del test | 2 – 6 |
...
| **3 · Las decisiones** | Ocho de las nueve rutas donde el naranja tiene prohibido aparecer —la novena, `calendario`, se migra en la tarea 20 con el resto de la plata | 22 – 30 |
...
**La tarea 6 es un gate, y nace verde.** Los cinco chequeos tienen que estar escritos y en verde
**antes** de que se toque la primera pantalla: el test existe para que la migración lo obedezca, y
si llega después sólo sirve para descubrir lo que ya se hizo mal.

Dos de los cinco describen el final y son falsos hoy: `.pill*` está escrito a mano en dos archivos
que se migran en la **tarea 37**, y catorce archivos importan de `@/lib/format` hasta la **tarea
41**. Por eso los dos **nacen con su lista de excepciones sembrada con exactamente esos dieciséis
archivos**, y de ahí sale la regla que gobierna toda la migración:

> **Cada tarea de pantalla saca sus archivos de la lista de excepciones en el mismo commit en que
> migra la pantalla.** La lista empieza con dieciséis entradas y termina vacía —salvo las que el
> chequeo admite para siempre, las que necesitan el string y no el elemento—. Si terminás una
> pantalla y el test sigue verde sin que hayas tocado la lista, la migraste a medias.

No se agrega ninguna excepción después de la tarea 6: sembrarla es un acto único, con el árbol de
hoy a la vista. Una entrada nueva significa una pantalla escrita sin las primitivas, que es
justamente lo que el chequeo existe para frenar.
...
| 8 ✅ | 1 | `app/(private)/page.tsx` deja de ser la pantalla de andamio (`text-4xl`, `min-h-screen`, dos enlaces subrayados) y pasa a `redirect('/tablero')`. No es contenido nuevo: es sacar del medio una pantalla que nunca fue del producto. **Lo pide `RF-24`**, dado de alta y firmado el 2026-08-31 tras el hallazgo H-6 de `/analyze` | `add_frontend_feature` | Developer | RF-24, RF-01, RF-02 |
...
| 13 ✅ | 1 | Test de render de `AuthLayout` (`RF-05`) y **recorrido de cierre de la ola 1** con Playwright: el shell, la raíz que redirige, y las cuatro pantallas de sesión, con el sistema operativo en modo oscuro para `RF-20` | `add_tests` | Tester | RF-01, RF-02, RF-05, RF-19, RF-20, RF-24 |
...
| 20 ✅ | 2 | `calendario/page.tsx` con `CalendarGrid` (536 loc, 10 botones, 7 llamadas a `money`): importes y días en mono, la señal del día vencido es la píldora, y **ningún naranja** —es una de las nueve rutas de decisión—. El `variant="brand"` del `confirm-dialog` que abre «Corregir» **se queda**: `RF-11` y `RF-21` exceptúan la ventana de confirmación | `add_frontend_feature` | Developer | RF-06, RF-09, RF-10, RF-19, RF-21 |
...
| # | Ola | Tarea | Skill | Rol | Cubre |
|---|-----|-------|-------|-----|-------|
| 6 ✅ | 0 | `frontend/tests/design-system.test.ts` pasa de 3 a 8 casos, todos por análisis estático —el mismo mecanismo que ya usa, que alcanza también a las pantallas que ningún test renderiza—: **(a)** presupuesto de naranja: por cada `app/(private)/**/page.tsx` se recorre su árbol de imports `@/` y se cuentan los `variant="brand"`, como mucho uno, **sin contar los de una ventana de confirmación** (`components/ui/confirm-dialog.tsx` queda fuera del recorrido: lo exceptúa la spec enmendada el 2026-08-31); **(b)** las nueve rutas de decisión —`revision`, `calendario`, `facturas/revision`, `facturas/incidentes`, `ventas/revision`, `proveedores/grafias`, `rubros/sin-clasificar`, `rubros/equivalencias`, `acciones`— dan cero, con la misma excepción del diálogo —sin ella `/calendario` daría uno, por el `confirm-dialog` que la 006 dejó en `main`—; **(c)** `.pill*` no aparece fuera de `badge.tsx` y `globals.css`, **sembrado con las dos excepciones de hoy** (`accesos/actividad/page.tsx` y `access/AccessTable.tsx`, que salen en la tarea 37); **(d)** `money`/`decimal`/`day` se importan de `@/lib/format` sólo desde `amount.tsx` y una lista de excepciones **por nombre de archivo**, sembrada con los catorce archivos que hoy lo hacen, que cada tarea de pantalla va sacando; **(e)** cero `dark:` y cero `prefers-color-scheme` fuera del comentario de cabecera de `globals.css`. Cada hallazgo reporta archivo y línea, para que un falso positivo se lea en un segundo. **Es el gate de toda la migración: los cinco escritos y en verde antes de la tarea 7**, dos de ellos por la lista sembrada que se vacía ola por ola | `add_tests` | Tester | RF-06, RF-09, RF-10, RF-11, RF-20, RF-21 |
...
|-----------|--------|------|
| RF-01 — toda la plataforma con la guía visual acordada | 2, 7, 8, 10, 35, 38, 39, 40, 44, y toda tarea de pantalla de las olas 2 a 4 | 13, 43 · más `UI-01`/`UI-02`, que ya rompen el build |
...
| RF-06 — un estado, una píldora, un color, en todas las pantallas | 3, 14, 15, 17, 18, 19, 20, 22, 24, 25, 31, 32, 33, 36, 37 | 6 *(la píldora sale de `Badge`)*, 21 *(la misma en tres pantallas)* |
| RF-07 — cinco significados de color, y el naranja no es uno | 3, 14 | 6 · el tipo de `tone` lo hace imposible de violar; 43 |
| RF-08 — lo no confirmado se distingue de lo confirmado | 2, 3, 14, 15, 16, 19, 25, 26, 28, 33 | 21, 43 |
| RF-09 — importes, fechas y códigos en mono tabular | 4, 14–20, 22–25, 32, 33, 34, 41 | 6 *(la plata pasa por el envoltorio)*, 21 |
| RF-10 — la columna de importes queda en columna | 4, 14, 15, 16, 17, 18, 19, 20, 23, 25, 34, 41 | 6, 21 *(cuatro dígitos contra siete)* |
| RF-11 — como mucho un naranja por pantalla, y es el de la tarea principal | 10, 11, 14, 15, 16, 17, 19, 31, 32, 33, 34, 35, 36, 37, 38, 39, 41 | 6 *(presupuesto de naranja, sin contar los diálogos)* |
| RF-12 — toda acción secundaria en contorno, gris o enlace | 11, 14, 15, 17, 19, 22–29, 33, 34, 36, 37 | 6, 30 |
...
| RF-15 — todo aviso lleva su acción | 5 *(`<Notice>` ya tiene `action` en su firma)*, 41 | 42, 43 · `UI-07` en el review |
...
| RF-19 — cargando, con error y sin resultados, iguales en todas partes | 5, 7, 11, 14, 18, 20, 22, 27, 29, 31, 32, 36, 37, 38, 40 | 13, 43 |
...
| RF-21 — cero naranja en las pantallas de decisión | 20 *(ola 2)*, 22, 23, 24, 25, 26, 27, 28, 29 | 6 *(las nueve rutas dan cero, sin contar la ventana de confirmación)*, 30 |
| RF-22 — Ventas es una entrada más del menú | 9 | 12 |
| RF-23 — fuera del tablero, cero exclusiones no muestran nada | 16 | 42 |
| RF-24 — al entrar se cae en el tablero, sin bienvenida en el medio | 8 | 13 *(recorrido de cierre de la ola 1: la raíz que redirige)* |
