...
| 25 ⬜ | **Medir la avalancha contra `staging` de producción** —consulta de sólo lectura, agrupada por el mismo criterio con el que la cola agrupa— **antes** de `/ship`. Contra los fixtures no sirve: se capturaron un día bueno | `add_tests` | Tester | — *(riesgo del plan)* |
