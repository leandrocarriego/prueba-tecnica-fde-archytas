...
| 24 ✅ | **Verificado, y la pata resultó cierta a medias.** El padrón y los comprobantes vuelven a abrir el caso solos en la siguiente lectura; **el buzón no**, y el motivo es fino: el `external_id` de un mensaje se compone con fecha, remitente y asunto, y una fecha que no se pudo leer compone el literal `"None|…"`, que es una clave tan buena como cualquier otra — así que el mensaje roto queda **conocido** desde la primera lectura y el filtro lo saca de todas las siguientes. Para lo que se aparte de acá en adelante da igual, porque el caso ya está abierto; lo que significa es que lo que el buzón publicó roto **antes** de estos suscriptores no aflora solo. Hoy no tiene consecuencia: la tarea 25 midió cero. Los dos hechos quedan fijados por test (`test_a_row_quarantined_before_opens_its_case_on_the_next_reading` y `test_the_inbox_is_the_one_that_does_not`), así que el día que alguien cambie el filtro o corra un backfill, el segundo falla y avisa que la decisión que lo justificaba cambió | `add_tests` | Tester | RF-01…RF-05 |
