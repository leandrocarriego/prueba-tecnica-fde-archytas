...
| II — Nada se descarta | Sí | **Es exactamente el artículo que esta feature termina de cumplir.** Hoy hay cinco orígenes que apartan a `staging` y no le avisan a nadie; después de esto, apartar y abrir caso son la misma acción en los siete |
...
una cola nueva**: le enchufa los cinco orígenes que le faltan y le agrega las tres cosas que la spec
...
RF-22 son un filtro: la ruta deja de pedir `Section.PRICES` —que hoy le cierra la pantalla a Marcela,
que es la dueña de cuatro de los cinco orígenes nuevos— y pasa a exigir sesión, mientras el servicio
...
| `triage` | Cuatro suscripciones nuevas (proveedores, pagos, mensajes, ventas) más la del cierre automático; columna `section` en `ExceptionCase`; filtro por áreas visibles en `list_cases`; chequeo de área en `resolve`; antigüedad y demora en los schemas; la ruta cambia de autorización | No |
...
| `sales` | Publica `QuarantinedSourceResolved` en `resolve_group` y `correct_sale` | No |
| `shared` | Cuatro eventos nuevos en `events/catalog.py`; el parámetro `triage.stale_days` en `parameters.py` | No |
| `frontend` | `/revision`: filtro por área, columna «espera hace», marca de demorado, total de pendientes, y las cuatro `kind` nuevas en `CASE_KINDS` | No |
...
- **`operations.exception.section`** — `Enum(BusinessSection)`, no nulo. Migración con backfill por
  `kind`: `unreadable_row`, `unknown_product`, `missing_product`, `unreadable_history`,
  `unknown_sale_row` → `SALES`; `unknown_category` → `PURCHASING` (la 010 puso los rubros del lado de
  compras, y `catalog/service.py::CATEGORY_SECTION` ya lo dice así — se lee de ahí, no se reinventa);
  todo lo demás → `PURCHASING`. Índice `ix_exception_section_status` para el listado filtrado, que es
  la consulta de la pantalla.
...
datetime | None` y `pending_total: int`. Los tipos del frontend se regeneran del OpenAPI, no se
escriben a mano.
...
|---|---|---|
| La cola se inunda el primer día: cuatro orígenes que nunca abrieron un caso van a abrir todo lo que tengan pendiente | Alto — una lista impracticable se abandona, que es el fracaso exacto que la spec quiere evitar | El agrupamiento por `fingerprint` ya existe y es lo que lo hace tolerable: cien filas rotas iguales son un caso con `occurrences=100`. Se mide en un dry-run contra los fixtures antes de shipear, y si el número sigue siendo grande es una conversación con el cliente, no un filtro escondido |
